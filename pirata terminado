...
//Creamos todas las variables que se van a usar en la matriz para crearla
void crearMatriz(char a[][20], int fila, int columna, int *pf, int *pc, int *tf, int *tc) {
    ...
    *tf = (rand() % (fila - 2)) + 1;
    ...
    for (int i = 0; i < fila; i++) {
        ...
        /* Agrego un else que asigna '_' el interior de la matriz 
        y dentro de ese else agrego 2 valores, uno que es un espacio 
        aleatorio donde se encuentre el pirata como 'P',
        y el otro para un espacio aleatorio donde se encuentre el tesoro como 'T'(Ahora esta oculta) */
    a[*pf][*pc] = 'P';
    a[*tf][*tc] = '_';
    a[0][columna - 1] = '-';
    a[fila - 1][0] = '-';
...
void jugarJuego(char a[][20], int fila, int columna, int pf, int pc, int tf, int tc) {
    ...
    int movimiento;
     ...
     //Creo un for que nos va a indicar que si la posicion del 
     //pirata es igual a la del tesoro al iniciar el juego, nos diga que ya ganamos
    if (a[pf][pc] == a[tf][tc]) {
